...
#------------------------------------------------------------------------------------
function Nseted_Wilson_loop_kx()
    nkx::Int64 = 100
    nky::Int64 = nkx
    hn::Int64 = 4
    Nocc::Int64 = Int(hn/2)
    ...
    for ix in 1:nkx
        ...
        for iy in 1:nky
            ky = kylist[iy]
            wann_v1, wann_v2 = Wilson_ky(ky) # 在固定ky的情况下，计算沿着kx方向的Wilson loop并得到对应的本征矢量
...
#-----------------------------------------------------------------------
function Nseted_Wilson_loop_ky()
    nkx = 100
    nky = 100
    hn = 4
    Nocc = Int(hn/2)
    ...
    for iy in 1:nky
        ...
        for ix in 1:nkx
            kx = kxlist[ix]
            wann_v1, wann_v2 = Wilson_kx(kx) # 在固定ky的情况下，计算沿着kx方向的Wilson loop并得到对应的本征矢量
...
#----------------------------------------------------------------------------------------------
function  Nested(num)
    x1,x2 = Nseted_Wilson_loop_ky()
    fx1 = "ky-" * string(num) * ".dat"
    f1 = open(fx1,"w")
    writedlm(f1,[x1 x2],"\t")
    close(f1)

    x1,x2 = Nseted_Wilson_loop_kx()
...
function test()
    x1,x2 = Wilson_kx(0.1)
    print(map(angle,x1))
    print(sort(map(angle,x1)))
    print(sortperm(map(angle,x1)))        
end
#-----------------------------------------------------------------
# @time test()
@time Nested(1)
