...
因此，一组$N$个Majorana模式对的希尔伯特空间$|\Psi\rangle$由交换费米子奇偶算符$P_n$的共同本征态$|s_1,s_2，\dots,s_N\rangle$所张开，并表示为
...
假设移动上图所示的轨迹需要的时间是T。在移动过程中，系统被描述为一个时间相关的哈密顿量$H(t)$， $0\leq t \leq T$。这个哈密顿量包含了系统的所有细节，比如Majorana所在的domain wall的位置。因为系统的最终构型与初始构型相同，例如，所有的域壁都位于与开始时相同的位置，所以此时有$H(0)=H(T)$(初末态总是相同)。换句话说，此时考虑的是一个将哈密顿量带回自身的闭合的移动。为了确保系统的波函数不离开基态流形$|\Psi\rangle$，我们需要缓慢的改变哈密顿量$H(t)$，使得演化过程服从[绝热定理](https://en.wikipedia.org/wiki/Adiabatic_theorem)。
