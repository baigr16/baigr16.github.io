...
当只考虑若干个近邻$\mathbf{R}_j$之后,此时就可以对公式(\ref{e4})中的求和
$\sum_{\mathbf{R}_j}$就得到了很大的简化.除了这个简化
...
的时候,其实不会去求对称化的基函数. 这里用于构建基函数的原子轨道波函数最好是晶
体所属点群的对称化波函数,此时我们就可以将对称操作操作用到这些波函数上面来化简
