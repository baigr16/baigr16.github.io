...
到这里基本的理论理论框架就建立起来了,剩下的问题就是怎么求解$H^\mathbf{k}_{\alpha\beta}$和
$S^\mathbf{k}_{\alpha\beta}$. 
...
当只考虑若干个近邻$\mathbf{R}_j$之后,此时就可以对公式(\ref{e4})中的求和$\sum_{\mathbf{R}_j}$就得到了很大的简化.除了这个简化
之外,还有对称性是可以利用的.我们构建TB模型就是为了研究一个确定的系统的性质,那么这个系统必然就是由对称性的,借助于对称性可以将
$\mathbf{R}_j$与其它位置$\mathbf{R}_i$联系起来,
那么这些通过对称性联系起来的位置处的交叠积分$E_{\alpha\beta}(\mathbf{R}_j)$进一步简化,那么此时哈密顿量矩阵中的独立参量的数量就减少了.
{:.success}
...
在实际计算和分析$E_{\alpha\beta}(\mathbf{R}_n)$的时候,其实不会去求对称化的基函数. 这里用于构建基函数的原子轨道波函数最好是晶
体所属点群的对称化波函数,此时我们就可以将对称操作操作用到这些波函数上面来化简$E_{\alpha\beta}(\mathbf{R}_j)$了.
