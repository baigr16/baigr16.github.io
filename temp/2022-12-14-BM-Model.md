...
$$
\begin{equation}
t_{\perp}(\mathbf{r})=\cos^{2}(\gamma)\ V_{pp\sigma}(\sqrt{d_{\perp}^{2}+\left|\mathbf{r}\right|^{2}})+\sin^{2}(\gamma)\ V_{pp\pi}(\sqrt{d_{\perp}^{2}+\left|\mathbf{r}\right|^{2}}),
\end{equation}
$$
...
$$
\begin{equation}
\cos^{2}(\gamma)=\frac{d_{\perp}^{2}}{d_{\perp}^{2}+\left|\mathbf{r}\right|^{2}},\quad\sin^{2}(\gamma)=\frac{\left|\mathbf{r}\right|^{2}}{d_{\perp}^{2}+\left|\mathbf{r}\right|^{2}}.
\end{equation}
$$
...
$$
\begin{equation}
t_{\perp}(\mathbf{p})=2\pi\int_{0}^{\infty}drrJ_{0}(\left|\mathbf{p}\right|r)t_{\perp}(r),
\end{equation}
$$
...
$$
\begin{equation*}
\left|\mathbf{q}_{1}\right|,\left|\mathbf{q}_{2}\right|\ll\left|K\right|
\end{equation*}
$$
...
这里对于$n=1(b),2(tr),3(tl)$有$\left|K_{1}+\mathbf{g}_{1,n}\right|=\left|K\right|=4\pi/(3a)$，在上面的方程中，定义了

$$
\begin{equation*}
T_{\mathbf{q}_{n}}^{\alpha\beta}=\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}e^{i\mathbf{g}_{1,n}\cdot\mathbf{\tau}_{1,\alpha}}e^{-i\mathbf{g}_{2,n}\cdot\mathbf{\tau}_{2,\beta}}
\end{equation*}
$$
...
$$
\begin{align}
T_{\mathbf{q}_{b}} & =\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}\left[\begin{array}{cc}
1 & 1\\
1 & 1
\end{array}\right],\\
T_{\mathbf{q}_{tr}} & =\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}e^{-i\mathbf{g}_{1,2}\cdot\mathbf{\tau}_{0}}\left[\begin{array}{cc}
e^{i\phi} & 1\\
e^{-i\phi} & e^{i\phi}
\end{array}\right],\\
T_{\mathbf{q}_{tl}} & =\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}e^{-i\mathbf{g}_{1,3}\cdot\mathbf{\tau}_{0}}\left[\begin{array}{cc}
e^{-i\phi} & 1\\
e^{i\phi} & e^{-i\phi}
\end{array}\right],
\end{align}
...
$$
\begin{align}
\mathbf{q}_{b} & =\left|\DeltaK\right|(0,-1),\\
\mathbf{q}_{tr} & =\left|\DeltaK\right|(\frac{\sqrt{3}}{2},\frac{1}{2}),\\
\mathbf{q}_{tl} & =\left|\DeltaK\right|(-\frac{\sqrt{3}}{2},\frac{1}{2}).
\end{align}
$$
...
$$
\begin{equation}
T_{12}^{\alpha\beta}(\mathbf{q}_{2},\mathbf{q}_{1})=\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}\left\{ \left[\begin{array}{cc}
1 & 1\\
1 & 1
...
e^{-i\phi} & 1\\
e^{i\phi} & e^{-i\phi}
\end{array}\right]\right\} =\frac{3t_{\perp}(\left|K\right|)}{A_{u.c.}}\left[\begin{array}{cc}
0 & 1\\
0 & 0
...
$$
\begin{equation}
t_{\perp}=\frac{3t_{\perp}(\left|K\right|)}{A_{u.c.}}.
\end{equation}
$$
...
$$
\begin{equation}
t_{\perp}(\left|K\right|)\simeq0.58 ev\cdot A^2
\end{equation}
$$
...
$$\begin{align}
\tilde{T}_{\mathbf{q}_{b}} & =\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}\left[\begin{array}{cc}
1 & 1\\
1 & 1
\end{array}\right],\\
\tilde{T}_{\mathbf{q}_{tr}} & =\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}e^{i\mathbf{g}_{1,2}\cdot\mathbf{\tau}_{0}}\left[\begin{array}{cc}
e^{-i\phi} & 1\\
e^{i\phi} & e^{-i\phi}
\end{array}\right],\\
\tilde{T}_{\mathbf{q}_{tl}} & =\frac{t_{\perp}(\left|K\right|)}{A_{u.c.}}e^{i\mathbf{g}_{1,3}\cdot\mathbf{\tau}_{0}}\left[\begin{array}{cc}
e^{i\phi} & 1\\
e^{-i\phi} & e^{i\phi}
\end{array}\right].
\end{align}
...
可以看到，这些态竟会与第二层晶体动量为$K_2+\mathbf{q}_2$的态耦合在一起，这里有三个可能的动量$\mathbf{q}_2$
...
$$
\begin{equation}
\left|1,K_{1}+\mathbf{q},\alpha\rangle,\left|2,K_{2}+\mathbf{q}+\mathbf{q}_{b},\alpha\rangle, \left|2,K_{2}+\mathbf{q}+\mathbf{q}_{tr},\alpha\rangle ,
\left|2,K_{2}+\mathbf{q}+\mathbf{q}_{tl},\alpha\rangle 
\end{equation}
$$
...
$$
\begin{equation}
\left[H_{2}^{K}(\mathbf{q})\right]^{-1}=(\mathbf{\sigma}^{\theta}\cdot\mathbf{q})/(v_{F}\hbar\left|\mathbf{q}\right|^{2})
\end{equation}
$$
...
$$
\begin{equation}
H_{1,\text{eff}}^{K}(\mathbf{q})=H_{1}^{K}(\mathbf{q})-\frac{1}{v_{F} \hbar \left|\DeltaK\right|^{2}}\sum_{n=1}^{3}\left[T_{\mathbf{q}_{n}}\cdot\mathbf{\sigma}^{\theta}\cdot T_{\mathbf{q}_{n}}^{\dagger}\right]\cdot(\mathbf{q}-\mathbf{q}_{n}+2\mathbf{q}_{n}\frac{\mathbf{q}_{n}\cdot\mathbf{q}}{\mathbf{q}_{n}^{2}}).
\end{equation}
$$
...
0 & q_{x}-iq_{y}\\
q_{x}+iq_{y} & 0
\end{array}\right]-6v_{F}\hbar\left|\Delta K\right|\sin(\frac{\theta}{2})\alpha^{2}\left[\begin{array}{cc}
1 & 0\\
0 & 1
...
$$
\begin{equation*}
\alpha=t_{\perp}(\left| K\right|)/(v_{F}\hbar\left|\Delta K\right|A_{u.c.})
\end{equation*}
$$
...
$$
\begin{equation}
\frac{v_{F}^{*}(\theta)}{v_{F}}=1-(\frac{t_{\perp}(\left| K\right|)}{v_{F}\hbar\left| K\right|A_{u.c.}})^{2}\frac{1}{4\sin^{2}(\theta/2)},
\end{equation}
$$

我们可以看到，有效的费米速度时依赖于转角的，对于特殊的转角$\theta$，费米速度会变的非常小，甚至可以变为零。对于很小的转角，其实$t_{\perp}\ll v_{F}\hbar\left|\mathbf{q}_{n}\right|$这个条件就并不再使用了，上面使用的微扰方法也并不成立，但是在特定的转角下，即魔角附近($\theta\sim1.05^\degree$)，费米速度将会变为零，转角双层石墨烯中就会出现平带。
...
$$
\begin{equation}
\left|\psi_{\mathbf{q}}^{m}\rangle =\sum_{l,\alpha,m_{1},m_{2}}u_{\alpha}^{(l)}(\mathbf{q},m_{1},m_{2})\left|l,\mathbf{q},(m_{1},m_{2}),\alpha\rangle ,\label{eq:Bloch-wave_expansion_tBLG}
\end{equation}
$$
...
$$
\begin{equation}
\left|l,\mathbf{q},(m_{1},m_{2}),\alpha\rangle \equiv\left|l,\text{K}_{l}+\mathbf{q}+m_{1}\mathbf{b}_{1}^{m}+m_{2}\mathbf{b}_{2}^{m},\alpha\rangle .
\end{equation}
$$
...
$$
\begin{equation}
\left|1,(0,0)\rangle ,\,\left|2,(0,0)\rangle ,\,\left|2,(0,1)\rangle \,\left|2,(-1,0)\rangle ,\,\left|1,(0,-1)\rangle ,\,\left|1,(1,0)\rangle ,\,\left|1,(0,1)\rangle ,\,\left|1,(1,1)\rangle ,\,\left|1,(-1,0)\rangle ,\,\left|1,(-1,-1)\rangle 
\end{equation}
$$
...
$$
\begin{equation*}
\left|\psi_{\mathbf{q}}^{m}\rangle =\sum_{l,\alpha,m_{1},m_{2}}u_{\alpha}^{(l)}(\mathbf{q},m_{1},m_{2})\left|l,\mathbf{q},(m_{1},m_{2}),\alpha\rangle 
\end{equation*}
$$

是通过参数$(m_1,m_2)$来控制选择的moir$\`e$倒格式的数量，其实也就是控制求和中的$m_1,m_2$。


在DFT的计算中，比如VASP就是利用平面波作为基函数来对本征态进行展开，并且会选取截断能$E\sim\mathbf{k}^2$，实际上截断对应的也就是晶体动量$\mathbf{k}$倒底要选择几倍的倒格式(也就是说要落在第几BZ中)，这一点就会与TBG中选择截断的moir$\`e$的数量是相似的。
...
而言，它可以是有限阶也可以是无限阶，与体系的转角有关系。对于一个公度系统(moir$\`e$周期可以表示为两个有理数的商)，此时两层之间的耦合只包含确定$\mathbf{G}^m$，此时$\left|\psi_{\mathbf{q}}^{m}\rangle$的展开就是有限阶的，**这里的有限阶**描述的系统是精确的。如果系统是非共度的，那么此时展开就会是无限阶的。对于TBG系统而言，在非公度时电子的性质其实与平面内两层相对平移矢量$\mathbf{\tau}_0$是无关的，这一点可以通过一个幺正变换来理解

$$
\begin{equation}
\left|l,\mathbf{q},(m_{1},m_{2}),\alpha\right\rangle \rightarrow e^{i\left(m_{1}\mathbf{b}_{1,1}+m_{2}\mathbf{b}_{1,2}\right)\cdot\mathbf{\tau}_{0}}\left|l,\mathbf{q},(m_{1},m_{2}),\alpha\right\rangle ,
\end{equation}
$$
