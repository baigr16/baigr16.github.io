...
$$
\begin{equation*}
\sum_{\mathbf{R}_{\ell}}e^{i\mathbf{k}\cdot\mathbf{R}_{\ell}}=N_{\ell}\sum_{\mathbf{G}_{\ell}}\delta_{\mathbf{k},\mathbf{G}_{\ell}}
\end{equation*}
$$
...
因为对$\mathbf{G}_1,\mathbf{G}_2$的求和是限制在第一BZ中的，因此在方程\eqref{eq:interlayer_hopping_Bloch_0}中将$\mathbf{G}_2\rightarrow-\mathbf{G}_2$是不影响结果的。这里体系的总面积$A=A_{u.c.1}N_{1}=A_{u.c.2}N_{2}$，$A_{u.c.\ell}$是每一层对应的原胞面积($A_{u.c.1}=A_{u.c.2}=A_{u.c.}=\sqrt{3}a^{2}/2$)，从而可以将上面的方程表示为
